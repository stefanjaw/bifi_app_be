...
export class CrEinvoiceReceptionService {
  ...
  async importReceived(
    ...
    const emisorContactType: "individual" | "company" =
      individualTipos.includes(emisorVatType) ? "individual" : "company";
      ...
      const [created] = await contactModel.create([
        {
          ...
          ...(validVatTypes.includes(emisorVatType)
            ? { crVatType: emisorVatType }
            : {}),
          active: true,
        },
      ]);
    ...
    const productModel =
      this.connectionManager.getModel<any>("InventoryProduct");
